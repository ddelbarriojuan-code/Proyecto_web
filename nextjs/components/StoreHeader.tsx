...
import { ShoppingCart, X, Search, Sun, Moon, User, LogOut, ClipboardList, Heart, LayoutGrid, List, GitCompare } from 'lucide-react'
...
function SecurityBadge(_props: SecurityBadgeProps) {
  return (
...
interface StoreHeaderProps {
  ...
  readonly comparadorCount?: number
  readonly onOpenComparador?: () => void
}
    ...
    <header>
      <div className="container header-main">
        <div className="header-left">
          <Link href="/" className="logo">Kratamex</Link>
          <SecurityBadge />
        </div>
        ...
        {authUser ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <Link href="/mis-pedidos" className="theme-toggle-btn" title="Mis pedidos"><ClipboardList size={16} /></Link>
            <Link href="/perfil" className="theme-toggle-btn" title="Mi perfil"><User size={16} /></Link>
            <button className="theme-toggle-btn" onClick={onLogout} title="Cerrar sesión"><LogOut size={16} /></button>
          </div>
        ) : (
          <Link href="/login" className="theme-toggle-btn" title="Iniciar sesión" style={{ textDecoration: 'none' }}>
            <User size={16} />
          </Link>
        )}

        {comparadorCount > 0 && (
          <button className="compare-header-btn" onClick={onOpenComparador} title="Comparar productos">
            <GitCompare size={17} />
